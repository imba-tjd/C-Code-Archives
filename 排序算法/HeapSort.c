...
void HeapSort(int array[], int num) {
    // 初建堆
    for (int i = num / 2; i > 0; i--) // 因为使用数组实现堆，num/2以后的都是叶子节点，直接认为排好了；num/2恰好是最后一个有孩子的节点（二叉树的性质）
...
void HeapAdjust2(int array[], int len, int k) {
    ...
    for (int i = 2 * k; i <= len; i *= 2) {
        ...
        if (t >= array[i])
            ...
            array[k] = array[i], k = i; // 把较小的与父节点交换，并把交换后的作为新的父节点
    }
    array[k] = t;
}
